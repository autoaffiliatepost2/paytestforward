...
from telethon.tl.types import MessageMediaPhoto, InputMediaPhoto
...
class TelegramForwarder:
    ...
    async def list_chats(self):
        ...
        for dialog in dialogs:
            print((f"Chat ID: {dialog.id}, Title: {dialog.title}").encode("utf-8"))
        # chats_file.write(f"Chat ID: {dialog.id}, Title: {dialog.title} \n")

        print("List of groups printed successfully!")

    async def forward_messages_to_channel(
        self, source_chat_id, destination_channel_id, keywords
    ):
        await self.client.connect()
        ...
        last_message_id = (await self.client.get_messages(source_chat_id, limit=1))[
            0
        ].id
        ...
        while True:
            ...
            messages = await self.client.get_messages(
                source_chat_id, min_id=last_message_id, limit=None
            )
            print(last_message_id)
            for message in reversed(messages):
                ...
                if message.text and any(
                    keyword in message.text.lower() for keyword in keywords
                ):
                ...
                if message.media and isinstance(message.media, MessageMediaPhoto):
                    # Check if the media has a spoiler
                    if getattr(message.media, "spoiler", None):
                        input_media = get_input_media(message.media)
                        input_media.spoiler = True
                    else:
                        input_media = get_input_media(message.media)

                    # Forward the photo message with or without spoiler
                    await self.client.send_file(
                        destination_channel_id, input_media, caption=message.text
                    )
                    ...
                    if message.text:
                        ...
                        await self.client.send_message(
                            destination_channel_id, message.text
                        )
                    ...
                    print("Message forwarded 1")
                # else:

                # if getattr(message.media, 'spoiler', None):
                #    message.media = get_input_media(message.media)
                #    message.media.spoiler=True

                #    await self.client.send_message(destination_channel_id, message.message)

                # else:
                ## Forward the message to the destination channel
                #    await self.client.send_message(destination_channel_id, message.text)

                # print("Message forwarded 2")
...
async def main():
    ...
    if choice == "1":
        ...
        source_chat_id = (
            -1001388213936
        )  ##-1001242185176 #-1001581472837 # -1001822686940  #int(input("Enter the source chat ID: "))
        destination_channel_id = (
            -1001822686940
        )  # int(input("Enter the destination chat ID: "))
        ...
        await forwarder.forward_messages_to_channel(
            source_chat_id, destination_channel_id, keywords
        )
    else:
